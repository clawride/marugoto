...
// Bản offline: VOICEVOX trên máy được máy chủ của chương trình chuyển tiếp qua /vvapi (cùng địa chỉ → không vướng CORS)
export const OFFLINE = process.env.NEXT_PUBLIC_OFFLINE === "1";
export const localUrl = () => (lsGet(LS.url, OFFLINE ? "/vvapi" : "http://127.0.0.1:50021") || "").trim().replace(/\/+$/, "");
...
const CHAT_BASE = process.env.NEXT_PUBLIC_VOICE_CHAT_BASE || "";
  ...
  if (!packs.has(id)) {
    packs.set(id, fetch(`/vn/voice/${id}.json`).then((r) => (r.ok ? r.json() : {})).catch(() => ({})).then((man) => {
      ...
      for (const e of Object.values(man)) {
        // f: tên file trong public/vn/voice/<id>/ — hoặc đường dẫn đầy đủ (GitHub Releases, tên đã có mã phiên bản)
        let url = /^https?:\/\//.test(e.f) ? e.f : `/vn/voice/${id}/${e.f}?v=${e.v}`;
        // bản offline: file trò chuyện (GitHub Releases) đã được chép vào máy
        if (CHAT_BASE) url = url.replace(/^https:\/\/github\.com\/[^/]+\/[^/]+\/releases\/download\/[^/]+\//, CHAT_BASE);
        for (const [h, [s, d]] of Object.entries(e.s || {})) m.set(h, { url, s, d });
      }
      return m;
    }));
  }
...
async function playSprite(p, rate, my) {
  ...
  // file ở xa (GitHub Releases) phải tải đoạn mới khi tua → chờ tua xong rồi mới phát, tránh play() bị ngắt
  if (a.seeking) await new Promise((res) => { a.addEventListener("seeked", res, { once: true }); setTimeout(res, 5000); });
  if (my !== seq) return;
  await a.play();
