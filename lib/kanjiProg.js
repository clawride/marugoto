...
// Nguồn: KANJIDIC2 (EDRDG, CC BY-SA 4.0) cho âm đọc/số nét/Hán Việt · KanjiVG (Ulrich Apel, CC BY-SA 3.0) cho nét và cấu tạo.
...
export const KIDX = META.idx; // { 漢: [vi, hv, lv] }
export { COMPS };
...
// Hán Việt của một từ = ghép Hán Việt từng chữ (chữ ngoài chương trình giữ nguyên)
export const hvOfWord = (w) => [...w].map((c) => KIDX[c]?.[1] || c).join(" ");
...
// 🧠 Ghi nhớ: chữ → nghĩa · nghĩa → chữ · chữ → Hán Việt · mẹo nhớ → chữ
export function memoQs(lesson, pool, n = 12) {
  ...
  list.forEach((E, i) => {
    ...
    const exp = `${E.k} — ${E.hv} · ${E.vi}. ${E.mnemonic || ""}`;
    let q = null;
    if (kind === 0) q = mc("Chữ này nghĩa là gì?", E.k, E.vi, [...(E.similar || []).map((s) => KIDX[s.k]?.[0]), ...others.map(vis)], { jpPrompt: true, explain: exp });
    else if (kind === 1) q = mc(`Chữ nào nghĩa là “${E.vi}”?`, `Âm Hán Việt: ${E.hv}`, E.k, lookAlikes(E, pool), { jpOpts: true, explain: exp });
    else if (kind === 2) q = mc("Âm Hán Việt của chữ này?", E.k, E.hv, others.map((x) => x.hv), { jpPrompt: true, explain: exp });
    else {
...
export function sentQs(lesson, pool, n = 12) {
  ...
  lesson.forEach((E, i) => {
    ...
    if (e && i % 2 === 0) {
      ...
      const q = mc("Chọn chữ Hán đúng cho phần gạch chân", e.jp.replace(w.w, `［${w.r}］`), w.w, fakes, { jpPrompt: true, jpOpts: true, explain: `${w.w} (${w.r}) — ${w.vi}. ${E.k}: ${E.hv} · ${E.vi}.` });
...
// Vần Hán Việt → đuôi âm On
const HV_END = [
  { re: /(ng|nh)$/i, cls: "UI", ends: ["ウ", "イ"], say: "vần -ng / -nh → âm On thường kết thúc bằng ウ hoặc イ" },
  { re: /(n|m)$/i, cls: "N", ends: ["ン"], say: "vần -n / -m → âm On thường kết thúc bằng ン" },
  { re: /(c|ch)$/i, cls: "K", ends: ["ク", "キ"], say: "vần -c / -ch → âm On thường kết thúc bằng ク hoặc キ" },
  { re: /t$/i, cls: "T", ends: ["ツ", "チ"], say: "vần -t → âm On thường kết thúc bằng ツ hoặc チ" },
];
const ALT_END = { UI: ["ン", "ク", "ツ"], N: ["ウ", "ク", "ツ"], K: ["ン", "ウ", "ツ"], T: ["ン", "ク", "ウ"] };
export const hvRule = (hv) => HV_END.find((r) => r.re.test(stripTone(hv)));
export function stripTone(s) { return (s || "").normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/đ/gi, "d").toLowerCase(); }
export function hvMatch(E) {
  const R = hvRule(E.hv);
  if (!R) return null;
  const on = (E.on || []).find((o) => R.ends.includes(o.slice(-1)) && o.length >= 2);
  return on ? { R, on } : null;
}
...
export function guessQs(lesson, pool, learned, n = 10) {
  ...
  lesson.forEach((E) => {
    ...
    // 2) Hán Việt → đuôi âm On
    const M = hvMatch(E);
    if (M) {
      const stem = M.on.slice(0, -1);
      const wrong = ALT_END[M.R.cls].map((e) => stem + e);
      const q = mc(`Hán Việt của ${E.k} là ${E.hv}. Âm On nào hợp nhất?`, `${E.k} · ${E.hv}`, M.on, wrong,
        { jpOpts: true, explain: `${E.hv}: ${M.R.say} → ${M.on}.` });
      if (q) qs.push(q);
    }
    // 3) bộ chỉ nghĩa → đoán nghĩa
    const rad = (E.comps || []).find((c) => c.rad && COMPS[c.el]?.vi) || (E.radical && COMPS[E.radical]?.vi ? { el: E.radical } : null);
    if (rad) {
      const others = pool.filter((x) => x.k !== E.k && !(x.comps || []).some((c) => c.el === rad.el) && x.radical !== rad.el);
      const q = mc(`Chữ ${E.k} có ${compName(rad.el)} (${COMPS[rad.el].vi}). Nghĩa nào hợp nhất?`, E.k, E.vi, shuffle(others).map(vis),
        { jpPrompt: true, explain: `${E.k} = ${E.vi}. ${COMPS[rad.el].hint || ""} ${E.explain || ""}` });
      if (q) qs.push(q);
    }
  });
  // 4) từ ghép: biết nghĩa từng chữ → đoán nghĩa cả từ
  const comp = uniq(lesson.flatMap((E) => (E.words || []).filter((w) => /^[一-鿿]{2}$/.test(w.w) && [...w.w].every((c) => learned.has(c))).map((w) => JSON.stringify(w))))
    .map((s) => JSON.parse(s));
  const allComp = pool.flatMap((E) => (E.words || []).filter((w) => /^[一-鿿]{2}$/.test(w.w)));
  shuffle(comp).slice(0, 4).forEach((w) => {
    const parts = [...w.w].map((c) => `${c} ${KIDX[c]?.[1] || ""} (${KIDX[c]?.[0] || ""})`).join(" + ");
    const q = mc(`Đoán nghĩa từ ghép ${w.w}`, parts, w.vi, shuffle(allComp.filter((x) => x.w !== w.w)).map((x) => x.vi),
      { explain: `${w.w} (${w.r}) — Hán Việt ${hvOfWord(w.w)} → ${w.vi}.` });
    if (q) qs.push(q);
  });
  return shuffle(qs).slice(0, n);
}
