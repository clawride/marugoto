...
// "bộ nước (ba chấm) · nước" — không lặp lại nghĩa nếu tên đã có
export const compLabel = (el) => { const n = compName(el), v = COMPS[el]?.vi; return v && !n.includes(v) ? `${n} (${v})` : n; };
...
export function guessQs(lesson, pool, learned, n = 10) {
  ...
  lesson.forEach((E) => {
    ...
    if (rad) {
      ...
      const q = mc(`Chữ ${E.k} có ${compLabel(rad.el)}. Nghĩa nào hợp nhất?`, E.k, E.vi, shuffle(others).map(vis),
