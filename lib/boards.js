...
// Chứng chỉ đã đỗ: B1-1 "1"…"3", A2-2 "A1"/"A2", A2/B1 "C1"/"C2" (thêm * nếu Xuất sắc)
const passedOf = (ex, pre = "") => Object.entries(ex || {}).filter(([, e]) => e.passed).map(([n, e]) => `${pre}${n}${e.excellent ? "*" : ""}`);
export const certsOf = (S) => [...passedOf(S.b1?.ex), ...passedOf(S.a22?.ex, "A"), ...passedOf(S.ab1?.ex, "C")];
