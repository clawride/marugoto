...
// Chứng chỉ đã đỗ: B1-1 "1"…"3", A2-2 "A1"/"A2" (thêm * nếu Xuất sắc)
const passedOf = (ex, pre = "") => Object.entries(ex || {}).filter(([, e]) => e.passed).map(([n, e]) => `${pre}${n}${e.excellent ? "*" : ""}`);
export const certsOf = (S) => [...passedOf(S.b1?.ex), ...passedOf(S.a22?.ex, "A")];
...
export function overallOf(S) {
  ...
  Object.values(S.a22?.p || {}).forEach((p) => (st += p.stars || 0));
  Object.values(S.a22?.boss || {}).forEach((b) => (st += b.best || 0));
  Object.values(S.a22?.ex || {}).forEach((e) => (st += (e.passed ? 5 : 0) + (e.excellent ? 5 : 0)));
