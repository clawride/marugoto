...
export const BOARD_GROUPS = [
  ...
  { key: "vqn", label: "📖 Từ vựng A1–A2/B1", boards: [["a1", "A1"], ["a21", "A2-1"], ["a22", "A2-2"], ["ab1", "A2/B1"]].flatMap(([b, nm]) => Array.from({ length: 9 }, (_, i) => ({ id: `vq:${b}-${i + 1}:all`, label: `${nm} · Topic ${i + 1}` }))) },
...
export const certsOf = (S) => [...passedOf(S.b1?.ex), ...passedOf(S.a22?.ex, "A"), ...passedOf(S.ab1?.ex, "C"), ...passedOf(S.a1?.ex, "E"), ...passedOf(S.a21?.ex, "D")];
...
export function entriesOf(S) {
  ...
  Object.entries(S.best || {}).forEach(([k, b]) => { const m = k.match(/^t((?:a1|a21|a22|ab1)-\d)_all$/); if (m) out.push({ board: `vq:${m[1]}:all`, pct: b.pct, total: b.t }); });
