...
export const BOARD_GROUPS = [
  ...
  { key: "vqn", label: "📖 Từ vựng A1–B1-2", boards: [["a1", "A1"], ["a21", "A2-1"], ["a22", "A2-2"], ["ab1", "A2/B1"], ["b12", "B1-2"]].flatMap(([b, nm]) => Array.from({ length: 9 }, (_, i) => ({ id: `vq:${b}-${i + 1}:all`, label: `${nm} · Topic ${i + 1}` }))) },
  ...
  { key: "b21", label: "⚡ Boss A2-1 (khóa đầy đủ)", boards: Array.from({ length: 9 }, (_, i) => ({ id: "b21:" + (i + 1), label: "Boss Topic " + (i + 1) })) },
  { key: "x21", label: "🎓 Chứng chỉ A2-1", boards: [1, 2].map((n) => ({ id: "x21:" + n, label: "Kỳ thi Cấp " + "I".repeat(n) + (n === 1 ? " (Bài 1–10)" : " (Bài 11–18)") })) },
  { key: "b12", label: "🔥 Boss B1-2", boards: Array.from({ length: 9 }, (_, i) => ({ id: "b12:" + (i + 1), label: "Boss Topic " + (i + 1) })) },
  { key: "x12", label: "🎓 Chứng chỉ B1-2", boards: [1, 2, 3].map((n) => ({ id: "x12:" + n, label: "Kỳ thi Cấp " + "I".repeat(n) + " (Topic " + (n * 3 - 2) + "–" + n * 3 + ")" })) },
...
export const certsOf = (S) => [...passedOf(S.b1?.ex), ...passedOf(S.a22?.ex, "A"), ...passedOf(S.ab1?.ex, "C"), ...passedOf(S.a1?.ex, "E"), ...passedOf(S.a21?.ex, "D"), ...passedOf(S.b12?.ex, "F")];
...
export function overallOf(S) {
  ...
  Object.values(S.kanji?.p || {}).forEach((p) => (st += p.stars || 0));
  for (const k of ["a22", "ab1", "a1", "a21", "b12"]) {
...
export function entriesOf(S) {
  ...
  Object.entries(S.best || {}).forEach(([k, b]) => { const m = k.match(/^t((?:a1|a21|a22|ab1|b12)-\d)_all$/); if (m) out.push({ board: `vq:${m[1]}:all`, pct: b.pct, total: b.t }); });
  ...
  Object.entries(S.a21?.boss || {}).forEach(([t, b]) => { if (b.total > 0) out.push({ board: "b21:" + t, pct: b.pct, total: b.total }); });
  Object.entries(S.a21?.ex || {}).forEach(([n, e]) => { if (e.best > 0) out.push({ board: "x21:" + n, pct: Math.round((e.best / 180) * 100), total: e.best }); });
  Object.entries(S.b12?.boss || {}).forEach(([t, b]) => { if (b.total > 0) out.push({ board: "b12:" + t, pct: b.pct, total: b.total }); });
  Object.entries(S.b12?.ex || {}).forEach(([n, e]) => { if (e.best > 0) out.push({ board: "x12:" + n, pct: Math.round((e.best / 180) * 100), total: e.best }); });
  return out;
}
