...
export const EXAMS = [
  { n: 1, topics: [1, 2, 3], name: "Chứng Chỉ B1-1 · Cấp I", sub: "Topic 1–3" },
  { n: 2, topics: [4, 5, 6], name: "Chứng Chỉ B1-1 · Cấp II", sub: "Topic 4–6" },
  { n: 3, topics: [7, 8, 9], name: "Chứng Chỉ B1-1 · Cấp III", sub: "Topic 7–9" },
];
...
export const scaled = (correct, total) => (total ? Math.round((correct / total) * 60) : 0);
export function verdict(scores) {
  const total = scores.lang + scores.read + scores.listen;
  const sectionFail = SECTIONS.filter((s) => scores[s.key] < PASS_SECTION).map((s) => s.key);
  const passed = total >= PASS_TOTAL && sectionFail.length === 0;
  return { total, passed, excellent: passed && total >= EXCELLENT, sectionFail };
}
