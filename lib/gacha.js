...
// - Sử Ký (Chronicled Wish): chọn 1 nhân vật/vũ khí 5★ chỉ định — 50% ra đúng, trượt thì lần 5★ sau chắc chắn (1 điểm định mệnh)
...
// Ước Nguyện Sử Ký — Inazuma
const CHRON_5C = ["Kamisato Ayaka", "Kamisato Ayato", "Yoimiya", "Chiori", "Yae Miko", "Arataki Itto", "Sangonomiya Kokomi", "Kaedehara Kazuha"];
const CHRON_5W = ["Mistsplitter Reforged", "Haran Geppaku Futsu", "Thundering Pulse", "Uraku Misugiri", "Kagura's Verity", "Redhorn Stonethresher", "Everlasting Moonglow", "Freedom-Sworn"];
const CHRON_4C = ["Thoma", "Sayu", "Gorou", "Kujou Sara", "Kuki Shinobu", "Shikanoin Heizou", "Kirara"];
...
export const POOL = {
  ...
  chron5c: byNames(CHARS, CHRON_5C),
  chron5w: byNames(WEAPONS, CHRON_5W),
  chron4c: byNames(CHARS, CHRON_4C),
...
export function currentBanners(t = Date.now(), pickId = null) {
  const p = phaseIndex(t);
  const picked = pickId && POOL.lim5c.find((c) => c.id === pickId);
  ...
  return {
    // Mặc định: nhân vật giới hạn mới nhất (giống banner đang chạy trong game)
    char: { featured5: picked || POOL.lim5c[0], featured4: uniq(pickN(POOL.all4c, 3, 3)) },
    chronicled: { chars5: POOL.chron5c, weapons5: POOL.chron5w, chars4: POOL.chron4c },
...
export const BANNER_INFO = {
  ...
  chronicled: { name: "Ước Nguyện Sử Ký · Inazuma", fate: "i", hard5: 90, soft5: 74, base5: 0.006, inc5: 0.06, hard4: 10, soft4: 9, base4: 0.051, inc4: 0.51 },
...
export const newBannerState = () => ({
  ...
  chronicled: { p5: 0, p4: 0, path: null, fp: 0 },
  standard: { p5: 0, p4: 0 },
});
...
// Lượt 5★ bắt buộc ra nhân vật (dùng cho cơ chế đặc biệt), vẫn theo luật của banner
export const BONUS_EVERY = 200;
export function rollForcedChar5(bannerId, st, banners) {
  st.p4 += 1;
  const pity = st.p5 + 1;
  st.p5 = 0;
  let r = roll5(bannerId, st, banners);
  if (r.kind !== "c") r = { kind: "c", x: pick(bannerId === "chronicled" ? banners.chronicled.chars5 : POOL.std5c), flags: {} };
  return { rank: 5, pity, ...r };
}
...
function roll5(bannerId, st, banners) {
  ...
  if (bannerId === "chronicled") {
    const C = banners.chronicled;
    const pool = [...C.chars5.map((x) => ({ kind: "c", x })), ...C.weapons5.map((x) => ({ kind: "w", x }))];
    const path = st.path ? pool.find((p) => `${p.kind}:${p.x.id}` === st.path) : null;
    if (!path) {
      // Chưa chọn chỉ định: 50% nhân vật / 50% vũ khí trong danh sách Sử Ký
      const r = Math.random() < 0.5 ? { kind: "c", x: pick(C.chars5) } : { kind: "w", x: pick(C.weapons5) };
      return { ...r, flags: {} };
    }
    if (st.fp >= 1 || Math.random() < 0.5) { st.fp = 0; return { ...path, flags: { path: true } }; }
    st.fp = 1;
    const others = pool.filter((p) => p !== path);
    return { ...pick(others), flags: { lost5050: true } };
  }
  // Thường trú
...
function roll4(bannerId, st, banners) {
  ...
  if (bannerId === "chronicled") {
    return Math.random() < 0.5 ? { kind: "c", x: pick(banners.chronicled.chars4), flags: {} } : { kind: "w", x: pick(POOL.std4w), flags: {} };
  }
  return { ...std4(), flags: {} };
}

/**
 * Tính phần thưởng phụ (Tinh Huy / Tinh Trần) & cung mệnh giống game.
...
// Số tầng cung mệnh có thể mở (bản trùng) và số tầng đã kích hoạt
export const consPotential = (S, key) => Math.min(6, Math.max(0, (S.inv?.[key] || 0) - 1));
export const consActive = (S, key) => Math.min(S.cons?.[key] || 0, consPotential(S, key));
export const stellaLeft = (S, key) => consPotential(S, key) - consActive(S, key);
