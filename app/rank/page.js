...
const CERT_KIND = { A: { cls: "a22", name: "A2-2", ico: "🌱A2-2·" }, C: { cls: "ab1", name: "A2/B1", ico: "🌊A2/B1·" }, E: { cls: "a1", name: "A1", ico: "🍃A1·" } };
function Certs({ c }) {
  if (!c) return null;
...
export default function RankPage() {
  ...
  const fmt = (score) => {
    ...
    if (/^(ex|x22|xb1|x01):/.test(board)) { const { pct, total } = decode(score); return <><b>{total}/180</b><small>{pct}%</small></>; }
